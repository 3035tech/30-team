...
- **DP leve** (`/dashboard?tab=dp`, Equipe → DP, `/employee/dp`) — ficha (CEP/ViaCEP, CPF, emergência), checklist documental com anexo S3, **assinatura interna** (traço no canvas via `signature_pad` + nome digitado + consentimento + auditoria; migrations `100`+`101`), férias com saldo + calendário 60d, absenteísmo na Overview e lembretes (`POST /api/cron/dp-doc-reminders`). Migrations `083`+`087`+`092`+`093`+`100`+`101`. **Não** é ICP / provedor externo / folha.
