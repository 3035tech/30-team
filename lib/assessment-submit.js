...
import { computeAssessmentFromAnswers } from './assessment-score';
import { upsertCandidate, normalizeEmail } from './ae/candidate-upsert';
...
} from './job-attribution';
import { scheduleJobFunnelEvent } from './job-funnel';
...
export async function submitAssessmentResult({
  ...
  if ((!companyToken && !vacancyToken) || !name || !areaKey || consent !== true) {
    return { ok: false, errorCode: 'INCOMPLETE_DATA', status: 400 };
  }
  ...
  if (vacancyToken) {
    ...
    if (link.rowCount === 0) {
      return { ok: false, errorCode: 'EXPIRED_LINK', status: 403 };
    }
    if (String(link.rows[0].status || '') === 'closed') {
      return { ok: false, errorCode: 'CLOSED_VACANCY', status: 403 };
    }
    companyId = link.rows[0].companyId;
    resolvedVacancyId = link.rows[0].vacancyId;
    if (!safeEmail) {
      return { ok: false, errorCode: 'REQUIRED_VACANCY_EMAIL', status: 400 };
    }
    ...
    if (inviteToken) {
      ...
      if (inv.rowCount === 0) {
        return { ok: false, errorCode: 'INVITE_INVALID', status: 400 };
      }
      ...
      if (invRow.inviteEmail !== safeEmail) {
        return { ok: false, errorCode: 'INVITE_EMAIL_MISMATCH', status: 400 };
      }
    ...
    companyId = link.rows[0].companyId;
    if (!safeEmail) {
      return { ok: false, errorCode: 'REQUIRED_CONTACT_EMAIL', status: 400 };
    }
  }
  ...
  if (areaRes.rowCount === 0) {
    return { ok: false, errorCode: 'INVALID_AREA', status: 400 };
  }
  ...
  if (resolvedVacancyId) {
    ...
    if (existingAssessment.rowCount > 0) {
      return { ok: false, errorCode: 'DUPLICATE_VACANCY_SUBMISSION', status: 409 };
    }
  }
