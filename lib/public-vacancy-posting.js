/**
 * Página pública da vaga — URL canônica `/vagas/{slug}-{id}` + SEO JobPosting.
 * Legado `/vaga/{companySlug}/{vacancySlug}` redireciona (301/308) para a canônica.
 */
...
/**
 * Path canônico: `/vagas/{slug}-{id}` (id = vacancies.id).
...
export function publicVacancyPath(a, b, c) {
  ...
  if (a != null && typeof a === 'object' && !Array.isArray(a)) {
    ...
    // (companySlug, vacancySlug, vacancyId) ou (vacancySlug, vacancyId)
    if (c != null) {
    ...
    // legado 2-arg sem id — não dá para montar canônica
    vacancySlug = String(b || a || '').trim();
    vacancyId = NaN;
  }
  if (!Number.isFinite(vacancyId) || vacancyId <= 0) return '/vagas';
  const slugPart = vacancySlug || 'vaga';
  return `/vagas/${encodeURIComponent(slugPart)}-${vacancyId}`;
}
...
/**
 * Parse `/vagas/{slug}-{id}` segment (ou só `{id}`).
...
export async function resolvePublicVacancyPostingById(vacancyId, expectedSlug = '') {
  ...
  if (!Number.isFinite(id) || id <= 0) return { ok: false, errorCode: 'NOT_FOUND', status: 404 };
...
export async function listOpenPublicVacancies(opts = {}) {
  ...
  const where = [
    ...
    `v.status = 'open'`,
    // Prazo vencido: fora do índice público / relacionadas
...
export function buildJobPostingJsonLd(posting, locale = 'pt-BR') {
  ...
  // Sem modalidade remoto/cidade no schema: não inventar TELECOMMUTE mundial.
