/**
 * Página pública da vaga — URL canônica `/j/{slug}-{id}` + SEO JobPosting.
 * Prefixo curto e neutro (pt/en), alinhado a `/t` `/v` `/r`.
 * Legado `/vagas/…` e `/vaga/{company}/{slug}` redirecionam (308) para a canônica.
 */
...
export async function resolvePublicCompanyBySlug(companySlug) {
  const slug = String(companySlug || '')
    .trim()
    .toLowerCase();
  if (!slug) return { ok: false, errorCode: 'NOT_FOUND', status: 404 };
