...
import { PAGE_SIZE_OPTIONS, sqlUsersOrderBy } from './assessment-filters';
import {
...
} from './user-password-invite';
import { resolveUserOrigin } from './user-signup-origin';
...
export async function createUser({
  ...
  if (!email) return { ok: false, errorCode: 'EMAIL_REQUIRED', status: 400 };
  if (!VALID_ROLES.has(role)) return { ok: false, errorCode: 'INVALID_ROLE', status: 400 };
  if (role !== 'admin' && !companyId) {
    return { ok: false, errorCode: 'COMPANY_REQUIRED', status: 400 };
  }

  if (wantInvite && !isMailConfigured()) {
    return { ok: false, errorCode: 'PASSWORD_OR_SMTP_REQUIRED', status: 400 };
  }
  ...
  if (companyId) {
    ...
    if (c.rowCount === 0) return { ok: false, errorCode: 'INVALID_COMPANY', status: 400 };
  }

  if (!wantInvite && suppliedPassword.length < 8) {
    return { ok: false, errorCode: 'PASSWORD_TOO_SHORT', status: 400 };
  }
...
export async function updateUser({
  ...
  if (current.rowCount === 0) return { ok: false, errorCode: 'USER_NOT_FOUND', status: 404 };

  const prev = current.rows[0];
  if (!assertUserInScope(prev, { isAdmin, companyId: scopeCompanyId })) {
    return { ok: false, errorCode: 'UNAUTHORIZED', status: 401 };
  }
  ...
  if (email !== null && !email) return { ok: false, errorCode: 'EMAIL_REQUIRED', status: 400 };
  if (role !== null && !VALID_ROLES.has(role)) {
    return { ok: false, errorCode: 'INVALID_ROLE', status: 400 };
  }
  ...
  if (nextRole !== 'admin' && !nextCompanyId) {
    return { ok: false, errorCode: 'COMPANY_REQUIRED_FOR_ROLE', status: 400 };
  }
  if (nextCompanyId) {
    const c = await query(`SELECT id FROM companies WHERE id = $1 AND deleted = FALSE LIMIT 1`, [
      nextCompanyId,
  ...
  if (Object.prototype.hasOwnProperty.call(body, 'modules')) {
    if (body.modules == null) {
      ...
      return { ok: false, errorCode: 'INVALID_MODULES', status: 400 };
    }
  } else {
...
export async function deactivateUser({
  ...
  if (actorUserId != null && Number(actorUserId) === Number(userId)) {
    return { ok: false, errorCode: 'CANNOT_DELETE_SELF', status: 400 };
  }
  ...
  if (del.rowCount === 0) return { ok: false, errorCode: 'USER_NOT_FOUND', status: 404 };
...
export async function resendUserPasswordInvite({
  ...
  if (cur.rowCount === 0) return { ok: false, errorCode: 'USER_NOT_FOUND', status: 404 };
