...
export async function suggestVacancyDescriptionAi({ vacancy, locale = 'pt-BR', mode = 'auto' }) {
  ...
  const system =
    resolved === 'improve'
      ? useEn
        ? `You improve vacancy descriptions for candidates. Keep facts; fix clarity and structure; do not invent benefits or salary. Follow the section guide. Return ONLY HTML.`
        : `Você melhora descrições de vaga para candidatos. Mantenha fatos; corrija clareza e estrutura; não invente benefícios nem salário. Siga o guia de seções. Devolva SÓ HTML.`
      : useEn
        ? `You draft vacancy descriptions for candidates from role facts. Use the section guide. Mark unknowns with [confirm with hiring manager]. No fake benefits. Return ONLY HTML.`
        : `Você redige descrições de vaga para candidatos a partir dos dados. Use o guia de seções. Marque o que faltar com [confirmar com o gestor]. Sem benefícios inventados. Devolva SÓ HTML.`;
