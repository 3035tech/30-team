...
function queueMobilePush({ body, candidateId, companyId, destination, title }) {
  queueMicrotask(() => sendMobileEmployeePush(null, { body, candidateId, companyId, destination, title })
    .catch((error) => console.error('mobile employee push delivery', error)));
}

function pushCopy(type, payload) {
  const copy = employeeNotificationCopySpec(type, payload);
  return { title: t('pt-BR', copy.titleKey, copy.values), body: t('pt-BR', copy.bodyKey, copy.values), destination: mobilePushDestinationFor(type) };
}
...
export async function notifyCandidate(dbOrQuery, opts) {
  ...
  try {
    ...
    if (dedupeKey) {
      ...
      if (r.rowCount) queueMobilePush({ candidateId: cand, companyId: cid, ...pushCopy(type, payload) });
