...
export const MODULE_TO_TABS = Object.freeze({
  [COMPANY_MODULE.CORE]: Object.freeze(['overview', 'team', 'help', 'profile', 'compensation']),
...
/**
 * For persist: all selectable checked → null (unrestricted / future modules auto-on).
...
export function modulesSelectionForPersist(selectedIds) {
  ...
  if (allOn || selected.size === 0) return null;
...
export function listCompanyModuleCatalog() {
  return COMPANY_MODULES.map((id) => ({
    id,
    required: id === COMPANY_MODULE_CORE,
    caps: MODULE_TO_CAPS[id] || [],
  }));
}
