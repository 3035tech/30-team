...
import { htmlToPlainText, isRichTextEmpty, sanitizeRichTextHtml } from './sanitize-html';
import {
...
function cleanConsultantNote(raw) {
  ...
  return plain.slice(0, CONSULTANT_NOTE_MAX_CHARS);
}

/**
 * Load vacancy + assessed candidates (with fit) for report building.
 * @returns {Promise<{ vacancy: object, people: object[], rubricWeights: object, rubricNotes: string|null } | null>}
 */
 ...
 * @param {{ executiveNote?: string, rubricWeights?: object, rubricNotes?: string|null, candidateOverrides?: Record<string, { recommendation?: string, note?: string }> }} opts
 */
  ...
  return {
    ...
    candidates: selectedPeople.map((p) => {
      ...
      return {
        ...
        consultantNote: cleanConsultantNote(ov.note),
        areaLabel: p.areaLabel || '',
        vacancyFitScore010: p.vacancyFitScore010,
        vacancyFitLabel: p.vacancyFitLabel,
        fitAlignedTypes: Array.isArray(p.fitAlignedTypes) ? p.fitAlignedTypes : [],
        fitGapTypes: Array.isArray(p.fitGapTypes) ? p.fitGapTypes : [],
      };
    }),
  };
}
