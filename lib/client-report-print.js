...
export function buildClientReportPrintHtml(opts = {}) {
  ...
  const logoUrl = String(vacancy.companyLogoUrl || data.companyLogoUrl || '').trim();
  ...
  const brandBlock = logoUrl
    ? `<div class="brand-row">${
        logoUrl
          ? `<img class="logo" src="${esc(logoUrl)}" alt="" width="48" height="48" />`
          : ''
      }${
        companyName
    ...
    : companyName
      ? `<p class="brand company-only">${esc(companyName)}</p>`
      : `<p class="brand">${esc(labels.product || '30Grow')}</p>`;
...
<head>
  ...
  <style>
    ...
    .brand-row { display: flex; align-items: center; gap: 10px; margin: 0 0 10px; }
    .brand-row .logo { object-fit: contain; border-radius: 6px; }
    .brand-row .company { font-family: ui-monospace, monospace; font-size: 12px; letter-spacing: 0.06em; text-transform: uppercase; color: #333; }
...
<body>
  ...
  <h1>${esc(title)}</h1>
  <p class="meta">${esc(
    [companyName && !logoUrl ? null : companyName, vacancy.positionsCount != null ? `${vacancy.positionsCount}` : '']
      .filter(Boolean)
      .join(' · ')
  )}</p>
  ...
  <p class="footer">${esc(labels.footer || labels.poweredBy || '')}</p>
</body>
</html>`;
}
...
export function printClientReport(opts = {}) {
  ...
  // Blob URL: works with noopener (blank about:blank + document.write does not).
  let url = '';
  try {
  ...
  const win = window.open(url, '_blank', 'noopener,noreferrer,width=820,height=900');
  if (!win) {
    try {
