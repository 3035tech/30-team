...
export async function ensurePreOnboardingChecklist(dbOrQuery, { companyId, candidateId }) {
  ...
  if (row.employmentStatus !== 'employee') {
  ...
  for (const key of PRE_ONBOARDING_KEYS) {
    const due = key === 'd1_welcome' ? addDaysIso(start, 1) : start;
    if (!due) continue;
    await db.query(
  ...
  try {
    const res = await db.query(
      `SELECT id, company_id AS "companyId", candidate_id AS "candidateId",
       ...
       ORDER BY
         CASE item_key
           WHEN 'email_access' THEN 1
           WHEN 'tools_access' THEN 2
           WHEN 'equipment' THEN 3
           WHEN 'd1_welcome' THEN 4
           ELSE 9
         END,
         id ASC`,
