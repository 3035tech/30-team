...
export async function upsertCompanyTimeSchedule(dbOrQuery, {
  ...
  const start = parseTimeHm(workdayStart, DEFAULT_SCHEDULE.workdayStart);
  const end = parseTimeHm(workdayEnd, DEFAULT_SCHEDULE.workdayEnd);
  if (hmToMinutes(end) <= hmToMinutes(start)) {
    return { ok: false, errorCode: ERR.INVALID_DATA };
  }
  const brk = Math.min(240, Math.max(0, Number(breakMinutes) || 0));
  const grace = Math.min(120, Math.max(0, Number(lateGraceMinutes) || 0));
  const tz = String(timezone || DEFAULT_SCHEDULE.timezone).trim().slice(0, 64) || DEFAULT_SCHEDULE.timezone;

  const r = await db.query(
    `INSERT INTO company_time_schedules (
       company_id, workday_start, workday_end, break_minutes, timezone,
       late_grace_minutes, updated_at, updated_by_user_id
     ) VALUES ($1, $2::time, $3::time, $4, $5, $6, NOW(), $7)
     ...
     RETURNING company_id AS "companyId",
               ...
               late_grace_minutes AS "lateGraceMinutes",
               updated_at AS "updatedAt"`,
    [cid, start, end, brk, tz, grace, updatedByUserId]
  );
