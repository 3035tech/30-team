...
export function shouldSuggestVariablePay(outcomes) {
  ...
  for (const o of entries) {
    ...
    if (!key) continue;
    total += 1;
    if (key === PERFORMANCE_GOAL_OUTCOME.EXCEEDED) {
      exceeded = true;
      good += 1;
    } else if (key === PERFORMANCE_GOAL_OUTCOME.MET) {
      good += 1;
    }
  }
...
export const VARIABLE_PAY_PROPOSED_CAP = 40;

/**
 * Company-wide inbox of proposed variable-pay events (RH approve/reject).
 */
export async function listProposedVariablePay(dbOrQuery, {
  companyId,
  limit = VARIABLE_PAY_PROPOSED_CAP,
} = {}) {
  const db = asDb(dbOrQuery);
  const cid = Number(companyId);
  ...
  const cap = Math.min(
    VARIABLE_PAY_PROPOSED_CAP,
    Math.max(1, Number(limit) || VARIABLE_PAY_PROPOSED_CAP)
  );
  const r = await db.query(
    `SELECT e.id, e.company_id AS "companyId", e.candidate_id AS "candidateId",
            e.event_type AS "eventType", e.amount,
            e.effective_date AS "effectiveDate", e.notes,
            e.approval_status AS "approvalStatus",
            e.source_review_id AS "sourceReviewId",
            e.source_cycle_id AS "sourceCycleId",
     ...
     WHERE e.company_id = $1
       AND e.approval_status = '${COMPENSATION_APPROVAL_STATUS.PROPOSED}'
       AND e.event_type IN ('bonus', 'other')
    ...
    `SELECT id, event_type AS "eventType", amount,
     ...
     WHERE company_id = $1 AND candidate_id = $2
       AND approval_status IN (
         '${COMPENSATION_APPROVAL_STATUS.PROPOSED}',
         '${COMPENSATION_APPROVAL_STATUS.APPROVED}'
       )
