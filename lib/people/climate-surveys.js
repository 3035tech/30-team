...
export async function updateClimateSurvey(dbOrQuery, {
  ...
  if (status !== undefined && nextStatus === 'open' && existing.status !== 'open' && !existing.opensAt) {
    nextOpens = nextOpens || new Date().toISOString();
  }
  if (status !== undefined && nextStatus === 'closed' && existing.status !== 'closed' && !existing.closesAt) {
...
export async function addClimateSurveyQuestion(dbOrQuery, {
  ...
  if (survey.questions.length >= QUESTIONS_CAP) return { ok: false, errorCode: 'QUESTIONS_CAP' };
  const text = String(prompt || '').trim().slice(0, PROMPT_MAX);
  if (!text) return { ok: false, errorCode: 'TITLE_REQUIRED' };
...
export async function updateClimateSurveyQuestion(dbOrQuery, {
  ...
  if (!nextPrompt) return { ok: false, errorCode: 'TITLE_REQUIRED' };
  const nextOrder =
...
export async function emailClimateSurveyInvites(dbOrQuery, {
  ...
  if (!list.length) return { ok: false, errorCode: 'INVALID_DATA' };
  if (!isMailConfigured()) return { ok: false, errorCode: 'MAIL_NOT_CONFIGURED' };
...
export async function getCompanyClimatePulse(dbOrQuery, { companyId }) {
  ...
  try {
    const res = await db.query(
      `SELECT
         COUNT(*) FILTER (WHERE s.status = 'open' AND s.deleted = FALSE)::int AS "openSurveys",
         COUNT(*) FILTER (WHERE s.status = 'draft' AND s.deleted = FALSE)::int AS "draftSurveys",
         COALESCE((
           SELECT COUNT(*)::int FROM climate_survey_responses r
           JOIN climate_surveys s2 ON s2.id = r.survey_id
           WHERE s2.company_id = $1 AND s2.status = 'open' AND s2.deleted = FALSE
         ), 0) AS "openResponses"
       FROM climate_surveys s
