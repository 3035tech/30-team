...
export async function createTeamPulse(dbOrQuery, {
  ...
  if (!(await assertGroupInCompany(db, { companyId, teamGroupId }))) {
    return { ok: false, errorCode: 'INVALID_GROUP' };
  }
...
export async function setTeamPulseStatus(dbOrQuery, { companyId, pulseId, status }) {
  ...
  if (!next) return { ok: false, errorCode: 'INVALID_STATUS' };
...
export async function createTeamPulseInvite(dbOrQuery, { companyId, pulseId }) {
  ...
  if (!pulse) return { ok: false, errorCode: 'NOT_FOUND' };
  if (pulse.status !== 'open') return { ok: false, errorCode: 'PULSE_NOT_OPEN' };
...
export async function getTeamPulseAggregate(dbOrQuery, { companyId, pulseId }) {
  const db = asDb(dbOrQuery);
  const pulse = await getTeamPulse(db, { companyId, pulseId });
...
export async function getPublicTeamPulseByToken(dbOrQuery, token) {
  ...
  if (tok.length < 16) return { ok: false, errorCode: 'NOT_FOUND' };
  const res = await db.query(
  ...
  if (row.deleted || row.status !== 'open') return { ok: false, errorCode: 'UNAVAILABLE' };
  if (row.usedAt) return { ok: false, errorCode: 'ALREADY_USED' };
...
export async function submitPublicTeamPulse(dbOrQuery, { token, answers }) {
  ...
  if (Object.keys(clean).length < 1) return { ok: false, errorCode: 'ANSWERS_REQUIRED' };
