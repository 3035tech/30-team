...
import { leaveInclusiveDays } from '../leave-days.js';

export { leaveInclusiveDays } from '../leave-days.js';
    ...
    const usage = await db.query(
      `SELECT
       ...
       WHERE company_id = $1
         AND candidate_id = $2
         AND leave_type = $3`,
      [cid, candId, DP_LEAVE_TYPE.VACATION]
    );
...
export async function listLeaveCalendar(dbOrQuery, { companyId, from, to, limit = 200 }) {
  ...
  return { items: res.rows.map(mapLeave), from: fromDate, to: toDate };
}
