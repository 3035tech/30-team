...
export async function getInterviewScorecard(dbOrQuery, {
  ...
  if (!Number.isFinite(vid) || !Number.isFinite(cid)) {
    return { ok: false, errorCode: 'INVALID_DATA', status: 400 };
  }

  const params = [vid, cid];
  let companyClause = '';
  if (!isAdmin) {
    if (companyId == null) return { ok: false, errorCode: 'UNAUTHORIZED', status: 401 };
...
export async function upsertInterviewScorecard(dbOrQuery, {
  ...
  if (!vac.rowCount) return { ok: false, errorCode: 'VACANCY_NOT_FOUND', status: 404 };
  ...
  if (!cand.rowCount) return { ok: false, errorCode: 'NOT_FOUND', status: 404 };
