...
import { buildDecisionBrief, scorePersonAgainstNucleus } from './decision-brief.js';
...
export async function buildCandidatePeopleBrief(dbOrQuery, opts) {
  ...
  const personTopType = Number(enneagram?.topType ?? topTypeHint);
  let nucleusFit = null;
  if (Number.isInteger(personTopType) && personTopType >= 1 && personTopType <= 9 && colleagues.length > 0) {
    nucleusFit = scorePersonAgainstNucleus({
      locale,
      person: { id: candidateId, topType: personTopType },
      nucleus: colleagues,
    });
  }
  ...
  return {
    motivatorsAttempt: motivatorsAttempt
      ? {
          ...
          profileSummary: motivatorsAttempt.profileSummary,
          ranking: Array.isArray(motivatorsAttempt.ranking)
            ? motivatorsAttempt.ranking.slice(0, 5)
            : [],
        }
      : null,
    management: hypotheses,
    decisionBrief: nucleusFit
      ? { ...decisionBrief, nucleusFit }
      : decisionBrief,
    nucleusFit,
    oneOnOnes,
