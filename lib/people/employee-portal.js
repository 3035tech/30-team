...
import { listDevelopmentPlans, getDevelopmentPlan } from './development-plans.js';
import { listOneOnOnes } from './one-on-ones.js';
import { buildManagementHypotheses } from './management-hypotheses.js';
import { listCandidateLmsCourses, completeLmsLesson } from '../lms.js';
import { ERR } from '../api-error-codes.js';
import { DEVELOPMENT_PLAN_STATUS } from '../domain-status.js';
...
export async function getEmployeePortalView(dbOrQuery, { token, locale = 'pt-BR' }) {
  ...
  const active = plans.filter((p) => p.status === DEVELOPMENT_PLAN_STATUS.ACTIVE).slice(0, 2);
  const planDetails = [];
  for (const p of active) {
    const full = await getDevelopmentPlan(db, {
      companyId: row.companyId,
      planId: p.id,
      candidateId: row.candidateId,
    });
  ...
  const oos = await listOneOnOnes(db, {
    companyId: row.companyId,
    candidateId: row.candidateId,
  });
  ...
  try {
    ...
    const enn = await db.query(
      `SELECT a.top_type AS "topType", a.scores
       ...
       WHERE a.candidate_id = $1 AND c.company_id = $2 AND a.completed_at IS NOT NULL
       ORDER BY a.completed_at DESC NULLS LAST, a.id DESC
       LIMIT 1`,
      [row.candidateId, row.companyId]
    );
  ...
  let courses = [];
  try {
    courses = await listCandidateLmsCourses(db, {
      companyId: row.companyId,
      candidateId: row.candidateId,
    });
  } catch {
