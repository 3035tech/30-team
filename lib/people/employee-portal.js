...
async function resolveActiveToken(db, token) {
  ...
  if (row.revokedAt) return { errorCode: 'REVOKED' };
  if (row.expiresAt && new Date(row.expiresAt) < new Date()) return { errorCode: 'EXPIRED' };
  return row;
}
...
export async function getEmployeePortalView(dbOrQuery, { token, locale = 'pt-BR' }) {
  ...
  const active = plans.filter((p) => p.status === 'active').slice(0, 2);
  const planDetails = [];
