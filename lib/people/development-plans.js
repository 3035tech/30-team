    ...
    const res = await db.query(
      `SELECT
         ...
         (
           ...
           WHERE p2.company_id = $1 AND p2.status = 'active' AND p2.deleted = FALSE
         ) AS "activeItems",
         (
           SELECT COUNT(*)::int
           FROM development_plan_items i
           JOIN development_plans p2 ON p2.id = i.plan_id
           WHERE p2.company_id = $1 AND p2.status = 'active' AND p2.deleted = FALSE AND i.status = 'done'
         ) AS "doneItems",
         (
           SELECT COUNT(*)::int
           FROM development_plan_items i
       ...
       FROM development_plans p
       WHERE p.company_id = $1 AND p.deleted = FALSE`,
      [cid]
    );
