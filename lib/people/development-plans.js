...
const PLAN_STATUSES = new Set(Object.values(DEVELOPMENT_PLAN_STATUS));
const ITEM_STATUSES = new Set(Object.values(DEVELOPMENT_PLAN_ITEM_STATUS));
const ITEM_SOURCES = new Set(DEVELOPMENT_PLAN_ITEM_SOURCES);
...
export async function listDevelopmentPlans(dbOrQuery, { companyId, candidateId, limit = LIST_CAP }) {
  ...
  const res = await db.query(
    `SELECT p.id, p.company_id AS "companyId", p.candidate_id AS "candidateId",
            ...
            (SELECT COUNT(*)::int FROM development_plan_items i WHERE i.plan_id = p.id AND i.status = '${DEVELOPMENT_PLAN_ITEM_STATUS.DONE}') AS "doneCount"
...
/**
 * Active plans + items in 2 queries (no N×getDevelopmentPlan).
 * Caps: planLimit active plans, itemsCap per plan when mapping.
 */
export async function listActiveDevelopmentPlansWithItems(
  dbOrQuery,
  { companyId, candidateId, planLimit = 2, itemsCap = 40 }
) {
  const db = asDb(dbOrQuery);
  const plansCap = Math.min(Math.max(1, Number(planLimit) || 2), 5);
  const perPlanItems = Math.min(Math.max(1, Number(itemsCap) || 40), ITEMS_CAP);
  const plans = await listDevelopmentPlans(db, {
    companyId,
    candidateId,
    limit: Math.max(plansCap * 2, 4),
  });
  const active = (plans || [])
  ...
  const planIds = active.map((p) => Number(p.id));
  const itemsR = await db.query(
    `SELECT i.id, i.plan_id AS "planId", i.company_id AS "companyId",
     ...
     WHERE i.company_id = $1 AND i.plan_id = ANY($2::bigint[])
     ORDER BY i.sort_order ASC, i.id ASC`,
    [companyId, planIds]
  );
  const byPlan = new Map();
  for (const it of itemsR.rows || []) {
    const pid = Number(it.planId);
    const list = byPlan.get(pid) || [];
    if (list.length < perPlanItems) list.push(it);
    byPlan.set(pid, list);
  }
  return active.map((p) => ({
    ...p,
    items: byPlan.get(Number(p.id)) || [],
  }));
}
...
export async function getDevelopmentPlan(dbOrQuery, { companyId, planId, candidateId = null }) {
  ...
  const itemIds = itemRows.map((r) => Number(r.id)).filter((n) => Number.isFinite(n) && n > 0);
  let linksByItem = new Map();
  let lmsByItem = new Map();
  if (itemIds.length > 0) {
    try {
      const links = await db.query(
        `SELECT dprl.plan_item_id AS "planItemId",
                lr.id, lr.title, lr.url, lr.theme,
         ...
         WHERE dprl.plan_item_id = ANY($1::int[])
           AND lr.company_id = $2
           AND lr.active = TRUE
      ...
      for (const row of links.rows || []) {
        const key = Number(row.planItemId);
        if (!linksByItem.has(key)) linksByItem.set(key, []);
        linksByItem.get(key).push({
          id: row.id,
          title: row.title,
          url: row.url,
          theme: row.theme,
    ...
    try {
      const lmsLinks = await db.query(
        `SELECT l.plan_item_id AS "planItemId", c.id AS "courseId", c.title
         ...
         WHERE l.plan_item_id = ANY($1::bigint[])
           AND c.company_id = $2
         ORDER BY l.created_at ASC`,
        [itemIds, companyId]
      );
      lmsByItem = new Map();
      for (const row of lmsLinks.rows || []) {
        const key = Number(row.planItemId);
        if (!lmsByItem.has(key)) lmsByItem.set(key, []);
        lmsByItem.get(key).push({
  ...
  return {
    ...
    items: itemRows.map((it) => ({
      ...it,
      linkedResources: linksByItem.get(Number(it.id)) || [],
      linkedLmsCourses: lmsByItem.get(Number(it.id)) || [],
    })),
  };
}
...
export async function createDevelopmentPlan(dbOrQuery, {
  ...
  title,
  objective = '',
  status = DEVELOPMENT_PLAN_STATUS.DRAFT,
  periodStart = null,
  periodEnd = null,
  ...
  const safeStatus = normalizeStatus(status, PLAN_STATUSES, DEVELOPMENT_PLAN_STATUS.DRAFT);
  ...
  for (let i = 0; i < ideas.length; i += 1) {
    const item = await insertPlanItem(db, {
      ...
      source: DEVELOPMENT_PLAN_ITEM_SOURCE.SYNTHESIS,
      sortOrder: i,
    });
...
async function insertPlanItem(db, {
  ...
  notes = '',
  status = DEVELOPMENT_PLAN_ITEM_STATUS.TODO,
  source = DEVELOPMENT_PLAN_ITEM_SOURCE.MANUAL,
  sortOrder = 0,
  dueDate = null,
  oneOnOneId = null,
  ownerLabel = '',
  ...
  if (oneOnOneId != null && oneOnOneId !== '') {
    ...
    if (Number.isFinite(n) && n > 0) {
      ...
      if (oo.rowCount === 0) return { ok: false, errorCode: ERR.INVALID_ONE_ON_ONE };
      ooId = n;
    }
  }
  const res = await db.query(
    ...
    [
      ...
      normalizeStatus(status, ITEM_STATUSES, DEVELOPMENT_PLAN_ITEM_STATUS.TODO),
      ITEM_SOURCES.has(String(source)) ? String(source) : DEVELOPMENT_PLAN_ITEM_SOURCE.MANUAL,
...
export async function addDevelopmentPlanItem(dbOrQuery, {
  ...
  if (!plan) return { ok: false, errorCode: ERR.NOT_FOUND };
  if (plan.items.length >= ITEMS_CAP) return { ok: false, errorCode: ERR.ITEMS_CAP };
...
export async function updateDevelopmentPlanItem(dbOrQuery, {
  ...
  if (oneOnOneId !== undefined) {
    if (oneOnOneId == null || oneOnOneId === '') {
      ...
      const n = Number(oneOnOneId);
      if (!Number.isFinite(n) || n <= 0) return { ok: false, errorCode: ERR.INVALID_ONE_ON_ONE };
      const oo = await db.query(
...
export async function importItemsFromOneOnOne(dbOrQuery, {
  ...
  if (!Number.isFinite(ooId) || ooId <= 0) return { ok: false, errorCode: ERR.INVALID_ONE_ON_ONE };
  ...
  if (lines.length === 0) return { ok: false, errorCode: ERR.NO_NEXT_STEPS };
  ...
  if (planId != null && planId !== '') {
    ...
    const active = await db.query(
      `SELECT id FROM development_plans
       WHERE company_id = $1 AND candidate_id = $2 AND status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}'
  ...
  if (room <= 0) return { ok: false, errorCode: ERR.ITEMS_CAP };
  ...
  for (let i = 0; i < Math.min(lines.length, room); i += 1) {
    const item = await insertPlanItem(db, {
      ...
      notes: '',
      status: DEVELOPMENT_PLAN_ITEM_STATUS.TODO,
      source: DEVELOPMENT_PLAN_ITEM_SOURCE.ONE_ON_ONE,
...
export async function getCompanyPdiPulse(
  ...
  try {
    const [totalsRes, overdueRes, unlinkedRes, noPlanRes, activePlansRes] = await Promise.all([
      db.query(
        `SELECT
           COUNT(*) FILTER (WHERE p.status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}')::int AS "activePlans",
           COUNT(DISTINCT p.candidate_id) FILTER (WHERE p.status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}')::int AS "peopleWithActive",
           (
             SELECT COUNT(*)::int
             FROM development_plan_items i
             JOIN development_plans p2 ON p2.id = i.plan_id
             WHERE p2.company_id = $1 AND p2.status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}'
           ) AS "activeItems",
           (
             SELECT COUNT(*)::int
             FROM development_plan_items i
             JOIN development_plans p2 ON p2.id = i.plan_id
             WHERE p2.company_id = $1 AND p2.status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}' AND i.status = '${DEVELOPMENT_PLAN_ITEM_STATUS.DONE}'
           ) AS "doneItems",
           (
             SELECT COUNT(*)::int
             FROM development_plan_items i
             JOIN development_plans p2 ON p2.id = i.plan_id
             WHERE p2.company_id = $1 AND p2.status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}'
               AND i.one_on_one_id IS NULL AND i.status <> '${DEVELOPMENT_PLAN_ITEM_STATUS.DONE}'
           ...
           (
             ...
             WHERE p3.company_id = $1 AND p3.status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}'
             ...
             WHERE c.company_id = $1
               ...
               AND NOT EXISTS (
                 ...
                 WHERE p4.candidate_id = c.id
                   ...
                   AND p4.status = '${DEVELOPMENT_PLAN_STATUS.ACTIVE}'
               )
        ...
        `SELECT
         ...
         LEFT JOIN LATERAL (
           SELECT
             ...
             COUNT(*) FILTER (WHERE i.status = '${DEVELOPMENT_PLAN_ITEM_STATUS.DONE}')::int AS done_count,
             COUNT(*) FILTER (
