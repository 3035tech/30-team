...
const ITEM_SOURCES = new Set(['manual', 'synthesis', 'one_on_one', 'retention']);
 ...
 */
export async function getCompanyPdiPulse(dbOrQuery, { companyId, queueLimit = PULSE_QUEUE_CAP } = {}) {
  const db = asDb(dbOrQuery);
  const cid = Number(companyId);
  if (!Number.isFinite(cid) || cid <= 0) return null;
  const cap = Math.min(Math.max(1, Number(queueLimit) || PULSE_QUEUE_CAP), PULSE_QUEUE_CAP);
  try {
    const [totalsRes, overdueRes, unlinkedRes, noPlanRes] = await Promise.all([
      db.query(
        `SELECT
