...
import { PIPELINE_STAGE_SET } from './pipeline';
...
export function parseDashboardTab(searchParams, sessionPayload) {
  ...
  if (!canAccessDashboardTab(sessionPayload, id)) {
    return canAccessDashboardTab(sessionPayload, 'overview') ? 'overview' : 'help';
  }
  return id;
}
...
const ROSTER_SCOPES = new Set(['internal', 'recruiting', 'all']);
...
export function parseRosterScope(searchParams) {
  ...
  return ROSTER_SCOPES.has(raw) ? raw : 'internal';
}
...
export function assessmentListWhereParts({
  ...
  const roster = ROSTER_SCOPES.has(rosterScope) ? rosterScope : 'internal';
