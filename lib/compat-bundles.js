 ...
 *   includePairs default=false — pair materialization é O(cap²); só a aba Compatibilidade precisa.
 *   Passar `includePairs: true` explicitamente quando o call site consome `pairs`/`tensions`/`synergies`.
 */
export function buildCompatBundles(lightRows, locale, opts = {}) {
  ...
  const includePairs = opts.includePairs === true;
