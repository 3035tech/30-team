...
import { buildTypeMixCompositionAdvice, dominantFromTypeCount } from './overview-type-mix.js';
...
export { buildTypeMixCompositionAdvice } from './overview-type-mix.js';
...
export async function buildOverviewMetrics(ctx) {
  ...
  try {
    ...
    let peopleOps = null;
    if (pulseCompanyId != null && Number(pulseCompanyId) > 0) {
      const [pdiPulse, climatePulse, onboardingPulse] = await Promise.all([
      ...
      if (pdiPulse || climatePulse || retentionCount > 0 || onboardingSignal) {
        peopleOps = {
          pdi: pdiPulse,
