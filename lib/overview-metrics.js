...
import { assessmentListWhereParts, sqlWhere } from './assessment-filters.js';
...
export async function buildOverviewMetrics(ctx) {
  ...
  try {
    ...
    const [
      ...
      queryRead(
        `SELECT
         ...
         LEFT JOIN LATERAL (
           SELECT
                  COUNT(*) FILTER (WHERE pipeline_stage = '${PIPELINE_STAGE.HIRED}') AS hired,
                  COUNT(*) FILTER (WHERE pipeline_stage NOT IN ('${PIPELINE_STAGE.REJECTED}', '${PIPELINE_STAGE.ARCHIVED}')) AS in_funnel,
                  COUNT(*) FILTER (
    ...
    if (pulseCompanyId != null && Number(pulseCompanyId) > 0) {
      const [pdiPulse, climatePulse, onboardingPulse, mixWindows, openRubrics] = await Promise.all([
