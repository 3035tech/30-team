...
import { getDpAttentionPulse } from './people/employee-dp.js';
...
export async function buildOverviewMetrics(ctx) {
  ...
  try {
    ...
    if (pulseCompanyId != null && Number(pulseCompanyId) > 0) {
      const [pdiPulse, climatePulse, enpsPulse, onboardingPulse, mixWindows, openRubrics, lmsOverduePulse, dpPulse] =
        await Promise.all([
