...
/** Soft apply na página pública: vaga aberta (link /v opcional). */
export function publicVacancyCanApply(posting, now = new Date()) {
  return !publicVacancyShowsClosedExperience(posting, now) && Boolean(posting?.vacancyId);
}
...
/** Data em máscara curta por locale (pt-BR dd/mm/aaaa · en mm/dd/yyyy). */
export function formatPublicVacancyDate(value, locale = 'pt-BR') {
  const formatted = formatDisplayDate(value, locale, { fallback: '' });
