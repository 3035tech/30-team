...
import { publicPricingCurrency, publicPricingTextValues } from './pricing-currency.js';
...
const COPY = {
  'pt-BR': {
    ...
    pricingSnapshotBody: 'As 20 primeiras empresas têm 90 dias. Depois, a partir de {monthlyPrice} por colaborador/mês.',
    ...
    faqs: [
      {
        q: 'É de graça mesmo?',
        a: 'Sim. Toda empresa começa com 30 dias grátis. As 20 primeiras têm 90 dias; design partners podem negociar até 6 meses. Depois, o plano começa em {monthlyPrice} por colaborador/mês, com 20% de desconto no anual.',
      },
      {
    ...
    earlyBody:
      'Toda empresa começa com 30 dias grátis. As 20 primeiras recebem 90 dias; design partners podem negociar até 6 meses. Depois, continue a partir de {monthlyPrice} por colaborador/mês.',
    earlyProof: [
  ...
  en: {
    ...
    pricingSnapshotBody: 'The first 20 companies get 90 days. Then from {monthlyPrice} per employee/month.',
    ...
    faqs: [
      {
        q: 'Is it really free?',
        a: 'Yes. Every company starts with 30 days free. The first 20 get 90 days; design partners can negotiate up to 6 months. Then plans start at {monthlyPrice} per employee/month, with 20% off annual billing.',
      },
      {
    ...
    earlyBody:
      'Every company starts with 30 days free. The first 20 get 90 days; design partners can negotiate up to 6 months. Then continue from {monthlyPrice} per employee/month.',
...
export function getProductLandingCopy(locale) {
  ...
  const loc = normalized === 'en' || normalized === 'es-419' ? normalized : 'pt-BR';
  const copy = COPY[loc];
  const { monthlyPrice } = publicPricingTextValues(normalized);
  const priceText = (text) => text.replaceAll('{monthlyPrice}', monthlyPrice);
  return {
    ...copy,
...
export function buildProductLlmsTxt() {
  ...
  const lines = [
    ...
    `Offer: 30-day trial; 90 days free for the first 20 companies. Brazil (pt-BR): ${publicPricingTextValues('pt-BR').monthlyPrice}; other locales: ${publicPricingTextValues('en').monthlyPrice} per employee/month.`,
...
export function buildProductLandingMetadata(locale = 'pt-BR') {
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: { 'pt-BR': url, en: url, 'es-419': url, 'x-default': url },
    ...
    openGraph: {
      ...
      description: copy.metaDescription,
      siteName: '30Grow',
      locale: normalizeLocale(locale) === 'en' ? 'en_US' : normalizeLocale(locale) === 'es-419' ? 'es_419' : 'pt_BR',
      alternateLocale: normalizeLocale(locale) === 'en' ? ['pt_BR', 'es_419'] : normalizeLocale(locale) === 'es-419' ? ['en_US', 'pt_BR'] : ['en_US', 'es_419'],
