...
function buildEmployeeMagicMail({ locale, personName, companyName, loginUrl, expiresMinutes }) {
  const pt = locale !== 'en';
  const subject = pt
    ? `Acesso 30Grow: ${companyName || 'sua empresa'}`
    : `30Grow access: ${companyName || 'your company'}`;
...
export async function issueEmployeePasswordInvite(dbOrQuery, opts = {}) {
  ...
  if (purpose === 'invite') {
    mail.subject = pt
      ? `Cadastre sua senha — ${companyLabel} · 30Grow`
      : `Set your password — ${companyLabel} · 30Grow`;
  } else {
    mail.subject = pt
      ? `Redefinir senha — ${companyLabel} · 30Grow`
      : `Reset password — ${companyLabel} · 30Grow`;
  }
  enqueueTransactionalMail({ to: toEmail, ...mail });
