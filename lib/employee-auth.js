...
export async function requestEmployeeMagicLink(dbOrQuery, opts = {}) {
  ...
  if (matches.length > 1 && !candidateId && companyId == null) {
    // Anti-enum: pede slug sem listar empresas.
    return { ok: true, sent: false, needsCompanySlug: true };
  }
...
export async function requestEmployeePasswordReset(dbOrQuery, { email, companyId = null, companySlug = null, locale = 'pt-BR' } = {}) {
  ...
  if (matches.length > 1 && scopedCompanyId == null) {
    return { ok: true, needsCompanySlug: true };
  }
  const person = matches[0];
  ...
  const issued = await issueEmployeePasswordInvite(db, {
    candidateId: person.candidateId,
    companyId: person.companyId,
    locale,
    purpose: 'reset',
...
export async function loginEmployeeWithPassword(dbOrQuery, { email, password, companyId = null, companySlug = null }) {
  ...
  if (matches.length > 1 && scopedCompanyId == null) {
    return { ok: false, errorCode: ERR.COMPANY_SLUG_REQUIRED, needsCompanySlug: true };
  }
