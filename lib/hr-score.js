 ...
 * Detecta piora de turnover (detectTrendChange) antes do upsert; notifica após save ok.
 */
export async function recalculateCompanyScores(companyId, { limit = 100 } = {}) {
  ...
  for (const row of res.rows) {
    try {
      // Antes do save — detectTrendChange lê turnover_risk anterior em hr_scores
      const change = await detectTrendChange(row.id);

      const scoreData = await calculateHrScore(row.id, cid);
      const predictions = await calculateAllPredictions(row.id, scoreData.signals);
      await saveHrScore(row.id, cid, scoreData, predictions);
