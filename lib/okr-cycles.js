...
import { EMPLOYMENT_STATUS, OKR_CYCLE_STATUS, OKR_CYCLE_STATUSES } from './domain-status.js';
...
function enrichAreas(areas, activitiesByArea) {
  return (areas || []).map((area) => {
    ...
    const progressPct = meanProgressPct(activities.map((a) => a.progressPct));
    return { ...area, activities, progressPct, activityCount: activities.length };
  });
}

function enrichCycle(cycle, areas) {
  const progressPct = meanProgressPct(
    (areas || []).map((a) => a.progressPct).filter((p) => p != null)
  );
