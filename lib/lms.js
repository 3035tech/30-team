...
export async function listLmsEnrollments(dbOrQuery, { companyId, courseId, limit = 80 }) {
  ...
  const enrollments = (r.rows || []).map((row) => {
    ...
    const dueIso = row.dueDate ? String(row.dueDate).slice(0, 10) : null;
    let overdue = false;
...
export async function enrollLmsCandidates(dbOrQuery, opts) {
  ...
  if (opts.dueDate != null && String(opts.dueDate).trim() !== '') {
    const s = String(opts.dueDate).trim().slice(0, 10);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) return { ok: false, errorCode: ERR.INVALID_DATE };
    dueDate = s;
  }
  ...
  if (opts.cohortName) {
    ...
    if (dueDate == null && co.rows[0].dueDate) {
      dueDate = String(co.rows[0].dueDate).slice(0, 10);
    }
  }
...
export async function completeLmsLesson(dbOrQuery, opts) {
  ...
  return {
    ...
    lessonId,
    courseId,
    newlyCompleted: Boolean(progress?.isComplete) && !wasComplete,
    ...progress,
  };
}
