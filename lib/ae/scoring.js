...
function normalizeQuestionType(q) {
  return q.questionType || q.question_type || '';
}
...
function buildQuestionRanges(questions) {
  ...
  for (const q of questions) {
    const w = q.weight || 1;
    const qRanges = { id: q.id, questionType: normalizeQuestionType(q), weight: w, dims: {} };

    if (normalizeQuestionType(q) === 'forced_choice') {
      const dimAccum = {};
    ...
    } else if (normalizeQuestionType(q) === 'likert') {
