...
export function computeMotivatorScores({ questions, answers }) {
  if (!Array.isArray(questions) || questions.length === 0) {
    return { ok: false, error: 'No questions to score.' };
  }
  ...
  for (const a of answers) {
    ...
    if (!expectedIds.has(qid)) {
      return { ok: false, error: `Invalid answer for question ${qid}.` };
    }
  ...
  if (answerByQ.size !== expectedIds.size) {
    return { ok: false, error: 'Please answer all questions in the session.' };
  }
  ...
  for (const q of questions) {
    ...
    if (isForcedChoice(q)) {
      ...
      if (!opt) return { ok: false, error: `Invalid option for question ${q.id}.` };
      const weights = opt.weights || {};
      if (Object.keys(weights).length === 0) {
        return { ok: false, error: `Missing weights for option ${optionId} (question ${q.id}). Run the seed.` };
      }
      ...
      if (order.length !== n) {
        return { ok: false, error: `Incomplete ranking for question ${q.id}.` };
      }
      ...
      if (seen.size !== n) {
        return { ok: false, error: `Invalid ranking for question ${q.id}.` };
      }
    } else if (isLikert(q)) {
      const val = answerLikertValue(answer);
      if (!Number.isFinite(val) || val < 1 || val > 5) {
        return { ok: false, error: `Invalid Likert value for question ${q.id}.` };
      }
      const dimensionWeights = q.dimensionWeights || {};
      if (Object.keys(dimensionWeights).length === 0) {
        return { ok: false, error: `Missing Likert weights for question ${q.id}. Run the seed.` };
      }
  ...
  if (maxNormalized === 0 && dimKeys.size > 0) {
    ...
    if (maxRaw === 0) {
      return {
        ok: false,
        error: 'Could not compute the profile: question weights are missing or invalid in the database.',
      };
    }
  }
