  ...
  const people = await dbQuery(
    `SELECT c.id AS "candidateId",
     ...
     WHERE c.company_id = $1
       AND (
         c.employment_status IN ('employee', 'alumni')
         OR EXISTS (
...
export async function batchCreateMotivatorsInvites(dbQuery, {
  ...
  const cid = Number(companyId);
  if (!Number.isFinite(cid)) {
    return { ok: false, errorCode: 'INVALID_COMPANY', status: 400, sent: [], skipped: [], failed: [] };
  }
  if (!appBaseUrl) {
    return { ok: false, errorCode: 'APP_URL_MISSING', status: 500, sent: [], skipped: [], failed: [] };
  }
  ...
  if (ids.length === 0) {
    return { ok: false, errorCode: 'NO_CANDIDATES', status: 400, sent: [], skipped: [], failed: [] };
  }
