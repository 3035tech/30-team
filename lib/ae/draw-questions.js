 ...
 * Quantidade padrão: 30 (14 forced choice + 4 ranking + 12 likert), conforme ae_definitions.config.
 */
...
/** Sorteia evitando concentrar demais o mesmo grupo (ex.: Likert da mesma dimensão). */
function pickRandomDiversified(items, count, groupKeyFn, maxPerGroup = 2) {
  if (!groupKeyFn || items.length <= count) return pickRandom(items, count);
  const copy = [...items];
  shuffleInPlace(copy);
  const chosen = [];
  const leftover = [];
  ...
  for (const item of copy) {
    const key = groupKeyFn(item) || '_';
    const n = used.get(key) || 0;
    if (chosen.length < count && n < maxPerGroup) {
      chosen.push(item);
      used.set(key, n + 1);
    } else {
      leftover.push(item);
...
function parseDrawConfig(definitionConfig = {}) {
  ...
  const questionsPerSession = Number(cfg.questions_per_session) || 30;
  const forcedChoicePerSession = Number(cfg.forced_choice_per_session) || 14;
  const rankingPerSession = Number(cfg.ranking_per_session) || 4;
  const likertPerSession = Number(cfg.likert_per_session) || 12;
...
export function drawQuestionsFromPool(allQuestions, definitionConfig) {
  const { forcedChoicePerSession, rankingPerSession, likertPerSession, shuffle } =
    parseDrawConfig(definitionConfig);

  const forced = allQuestions.filter((q) => q.questionType === 'forced_choice');
  const ranking = allQuestions.filter((q) => q.questionType === 'ranking');
  const likert = allQuestions.filter((q) => q.questionType === 'likert');

  const drawnForced = pickRandom(forced, forcedChoicePerSession);
  const drawnRanking = pickRandom(ranking, rankingPerSession);
  const drawnLikert = pickRandomDiversified(likert, likertPerSession, (q) => q.category, 2);
  const drawn = [...drawnForced, ...drawnRanking, ...drawnLikert];
...
export async function drawMotivatorsQuestions(dbOrQuery, definitionSlug = 'motivators') {
  ...
  if (defRes.rowCount === 0) {
    return { ok: false, errorCode: ERR.DEFINITION_NOT_FOUND };
  }
  ...
  if (qRes.rowCount === 0) {
    return { ok: false, errorCode: ERR.QUESTION_BANK_EMPTY };
  }
