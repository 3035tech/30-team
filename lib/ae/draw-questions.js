 ...
 */
export async function drawMotivatorsQuestions(db, definitionSlug = 'motivators') {
  ...
  for (const row of optRes.rows) {
    if (!optionsByQuestion.has(row.questionId)) {
      optionsByQuestion.set(row.questionId, []);
    }
    optionsByQuestion.get(row.questionId).push(mapOptionRow(row));
  }

  const likertByQuestion = new Map();
  for (const row of likertRes.rows) {
    if (!likertByQuestion.has(row.questionId)) {
      likertByQuestion.set(row.questionId, {});
    }
    likertByQuestion.get(row.questionId)[row.dimensionKey] = parseFloat(row.weightPerPoint) || 0;
  }
  ...
  const pool = qRes.rows.map((q) => {
    ...
    if (q.questionType === 'forced_choice') {
      return { ...base, options: optionsByQuestion.get(q.id) || [] };
    }
    return { ...base, dimensionWeights: likertByQuestion.get(q.id) || {} };
  });
