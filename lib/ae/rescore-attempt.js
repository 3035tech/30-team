...
export async function rescoreAttemptData(dbOrQuery, { questionIds, answers, definitionId, locale = 'pt-BR', repairWeights = false }) {
  ...
  if (!ids.length) return { ok: false, error: 'Session has no questions.' };
  if (!answerList.length) return { ok: false, error: 'No answers saved for this attempt.' };
  ...
  if (diagnostics.questionsLoaded === 0) {
    return {
      ok: false,
      error: `No active question found for the ${ids.length} session IDs.`,
      diagnostics,
    };
  }
...
export async function maybeRescoreAndPersist(dbOrQuery, attemptId, { locale = 'pt-BR', force = false } = {}) {
  ...
  if (res.rowCount === 0) return { rescored: false, error: 'Attempt not found.' };
  ...
  if (newMax === 0) {
    return { rescored: false, error: result.error || 'Could not compute scores (> 0).', diagnostics: result.diagnostics };
  }
