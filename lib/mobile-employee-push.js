...
export async function sendMobileEmployeePush(dbOrQuery, { candidateId, companyId, title, body, destination }) {
  const db = asDb(dbOrQuery || query);
  let tokens;
  try {
  ...
  try {
    const response = await fetch(EXPO_PUSH_URL, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify(tokens.map((to) => ({ to, title, body, sound: 'default', data: { destination, url: `team30://workspace?destination=${encodeURIComponent(destination)}` } }))), signal: controller.signal });
