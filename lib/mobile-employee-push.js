...
export async function sendMobileEmployeePush(dbOrQuery, { candidateId, companyId, destination, notificationId }) {
  ...
  try {
    const response = await fetch(EXPO_PUSH_URL, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify(tokens.map((to) => ({ to, title: '30 Team', body: 'Você tem uma atualização no app.', sound: 'default', data }))), signal: controller.signal });
