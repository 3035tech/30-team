...
const EXIT_TYPES = new Set(['voluntary', 'involuntary', 'mutual']);
...
export async function createExitRecord(dbOrQuery, {
  ...
  if (cand.rowCount === 0) return { ok: false, errorCode: 'CANDIDATE_NOT_FOUND' };
  if (cand.rows[0].employmentStatus !== 'employee') {
    return { ok: false, errorCode: 'NOT_EMPLOYEE' };
  }

  const safeExitDate = dateOrNull(exitDate) || new Date().toISOString().slice(0, 10);
  const safeType = normalizeStatus(exitType, EXIT_TYPES, 'voluntary');
  const safeReason = normalizeStatus(exitReason, EXIT_REASONS, 'other');
  ...
  try {
    ...
    await db.query(
      `UPDATE candidates SET employment_status = 'alumni' WHERE id = $1 AND company_id = $2`,
      [candidateId, companyId]
    );

    return { ok: true, exitRecord: res.rows[0] };
  } catch (err) {
    if (err?.code === '23505') return { ok: false, errorCode: 'EXIT_ALREADY_RECORDED' };
    throw err;
  }
}
...
export async function listExitRecords(dbOrQuery, { companyId, limit = LIST_CAP }) {
  ...
  const res = await db.query(
    `SELECT e.id, e.candidate_id AS "candidateId", e.exit_date AS "exitDate",
            e.exit_type AS "exitType", e.exit_reason AS "exitReason",
            c.full_name AS "candidateName", c.email AS "candidateEmail",
...
export async function updateExitRecord(dbOrQuery, {
  ...
  const nextDate = exitDate !== undefined ? dateOrNull(exitDate) : row.exitDate;
  const nextType = exitType !== undefined ? normalizeStatus(exitType, EXIT_TYPES, row.exitType) : row.exitType;
  const nextReason = exitReason !== undefined ? normalizeStatus(exitReason, EXIT_REASONS, row.exitReason) : row.exitReason;
...
export async function getExitInsights(dbOrQuery, { companyId }) {
  ...
  // Insight 1: High voluntary exits for better_offer/compensation → M1 (recruitment: compensation not competitive)
  const compReasons = reasonAgg.filter((r) =>
    r.exitType === 'voluntary' && ['better_offer', 'compensation'].includes(r.exitReason)
  );
  ...
  if (compCount / total > 0.3) {
    insights.push({
      ...
      signal: 'compensation',
      severity: 'high',
      description: 'Alto volume de saídas por compensação/proposta melhor',
  ...
  // Insight 3: High manager_relationship exits → M3/M4 (management issue)
  const managerIssue = reasonAgg.find((r) => r.exitReason === 'manager_relationship');
  if (managerIssue && managerIssue.count / total > 0.15) {
    insights.push({
      category: 'management',
      signal: 'manager_relationship',
      severity: 'high',
      description: 'Relacionamento com gestor é motivo recorrente de saída',
      suggestion: 'Investir em 1:1, feedback contínuo e desenvolvimento de liderança',
      count: managerIssue.count,
      percentage: Math.round((managerIssue.count / total) * 100),
    });
  }

  // Insight 4: High career_growth exits → M3/M4 (lack of development)
  const careerGrowth = reasonAgg.find((r) => r.exitReason === 'career_growth');
  if (careerGrowth && careerGrowth.count / total > 0.2) {
    insights.push({
      category: 'management',
      signal: 'career_growth',
      severity: 'medium',
      description: 'Falta de crescimento/desenvolvimento é motivo comum',
      suggestion: 'Estruturar PDI, sucessão e plano de carreira visível',
      count: careerGrowth.count,
      percentage: Math.round((careerGrowth.count / total) * 100),
    });
  }

  // Insight 5: Involuntary performance exits > 30% → M1 or M3 (hiring or onboarding issue)
