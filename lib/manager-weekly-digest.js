  ...
  const [
    ...
    db.query(
      `SELECT COUNT(*)::int AS n
       ...
       WHERE c.company_id = $1
         AND ass.pipeline_stage = 'interview'
    ...
    db.query(
      `SELECT COUNT(*)::int AS n
       ...
       WHERE c.company_id = $1
         AND ass.pipeline_stage = 'approved'
         AND (
    ...
    db.query(
      `SELECT COUNT(*)::int AS n
       FROM vacancies v
       WHERE v.company_id = $1 AND v.deleted = FALSE AND v.status = 'open'
...
export async function runManagerWeeklyDigest(dbOrQuery, {
  ...
  for (const { id: companyId } of companies.rows) {
    const [retention, stale, attention] = await Promise.all([
      db.query(
    ...
    if (retentionCount === 0 && staleCount === 0 && attentionTotal === 0) continue;
