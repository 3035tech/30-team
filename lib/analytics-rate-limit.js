 ...
 */
export function checkAnalyticsRateLimit(request, scope) {
  if (!scope || !scope.userId) {
    // Sem scope válido = sem rate limit (auth vai falhar antes)
    return null;
  }

  const key = `analytics:u${scope.userId}`;
  const result = checkRateLimit(key, ANALYTICS_LIMIT, ANALYTICS_WINDOW_MS);

  if (!result.ok) {
    const response = apiError(request, 'RATE_LIMIT', 429);
  ...
  // OK - adicionar headers informativos à resposta (caller deve aplicar)
  return null;
}
...
export function addRateLimitHeaders(response, scope) {
  ...
  // Estimar remaining (não é 100% preciso sem consultar bucket novamente)
  response.headers.set('X-RateLimit-Limit', String(ANALYTICS_LIMIT));
  // Aproximação: não temos count exato aqui, então omitir X-RateLimit-Remaining
  // ou fazer uma segunda consulta ao bucket (overhead)
  return response;
}
