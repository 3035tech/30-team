...
import { parseVacanciesSort, sqlVacancyOrderBy } from './assessment-filters';
import { parseVacancyDetailsFromBody } from './vacancy-details';
import { ensureActiveVacancyLinkToken } from './vacancy-link';
import { slugify } from './slugify';
import { sanitizeInterviewNotesHtml } from './sanitize-html';
import { scheduleVacancyIndexSync } from './job-indexing';
import { scheduleJobAlertDispatch } from './job-alerts';
import { notifyCompanyManagers, NOTIF } from './manager-notifications';
import { upsertCandidatePreInterview } from './ae/candidate-upsert';
import { normalizeCandidateProfile } from './candidate-profile';
...
export async function createVacancy({
  ...
  if (c.rowCount === 0) return { ok: false, errorCode: 'INVALID_COMPANY' };
...
export async function updateVacancy({ vacancyId, current, body }) {
  ...
  if (checkMin != null && checkMax != null && Number(checkMin) > Number(checkMax)) {
    return { ok: false, errorCode: 'INVALID_SALARY_RANGE' };
  }

  if (status !== null && !['open', 'closed'].includes(status)) {
    return { ok: false, errorCode: 'INVALID_STATUS' };
  }
  if (slug !== null && !slug) return { ok: false, errorCode: 'INVALID_SLUG' };
  ...
  if (body.vacancyFitWeights != null || body.vacancyRubricNotes !== undefined) {
    ...
    if (typeof w !== 'object' || Array.isArray(w)) {
      return { ok: false, errorCode: 'INVALID_FIT_WEIGHTS' };
    }
    const notes =
...
export async function softDeleteVacancy({ vacancyId, beforeDelete }) {
  ...
  if (del.rowCount === 0) return { ok: false, errorCode: 'NOT_FOUND' };
...
export async function linkVacancyCandidate({
  ...
  if (existingCandidateId) {
    ...
    if (!existing.rowCount) return { ok: false, errorCode: 'NOT_FOUND', status: 404 };
    ...
    if (!email || !EMAIL_RE.test(email)) {
      return { ok: false, errorCode: 'INVALID_CANDIDATE_EMAIL', status: 400 };
    }
...
export async function loadVacancyForActor({ vacancyId, isAdmin, companyId }) {
  if (!isAdmin && !companyId) return { ok: false, errorCode: 'UNAUTHORIZED' };
  ...
  if (!isAdmin) {
    ...
    if (owned.rowCount === 0) return { ok: false, errorCode: 'UNAUTHORIZED' };
  ...
  if (exists.rowCount === 0) return { ok: false, errorCode: 'VACANCY_NOT_FOUND' };
