...
export { ERR, HTTP_STATUS, httpStatusForError } from './api-error-codes.js';

/**
 * Resolve o locale a partir do cookie NEXT_LOCALE.
 * Aceita um `Request`/`NextRequest` (lê via `request.cookies`) ou nenhum argumento
 * (sem request, usa o locale padrão; Route Handlers devem passar o request).
 */
 ...
 *   um locale já resolvido ('pt-BR'|'en'), ou `null`/`undefined` (usa o padrão).
