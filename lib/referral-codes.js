...
export async function createReferralCode(input) {
  ...
  if (!Number.isFinite(companyId) || companyId <= 0) {
    return { ok: false, errorCode: 'INVALID_COMPANY' };
  }
  ...
  if (input.vacancyId != null && input.vacancyId !== '') {
    ...
    if (!Number.isFinite(vacancyId) || vacancyId <= 0) {
      return { ok: false, errorCode: 'INVALID_VACANCY' };
    }
    ...
    if (vac.rowCount === 0) return { ok: false, errorCode: 'INVALID_VACANCY' };
  }

  let code = input.code ? normalizeReferralCode(input.code) : null;
  if (input.code && !code) return { ok: false, errorCode: 'INVALID_DATA' };
  ...
  for (let attempt = 0; attempt < 6; attempt++) {
    ...
    try {
      ...
      if (err?.code === '23505') {
        if (code) return { ok: false, errorCode: 'DUPLICATE_REFERRAL_CODE' };
        continue;
      }
      throw err;
    }
  }
  return { ok: false, errorCode: 'DUPLICATE_REFERRAL_CODE' };
}
...
export async function updateReferralCode(input) {
  ...
  if (!Number.isFinite(id) || id <= 0) return { ok: false, errorCode: 'INVALID_ID' };
  ...
  if (!sets.length) return { ok: false, errorCode: 'INVALID_DATA' };
  ...
  if (r.rowCount === 0) return { ok: false, errorCode: 'NOT_FOUND' };
