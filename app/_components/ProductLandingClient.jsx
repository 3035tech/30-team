...
function EmployeeAppPreview({ copy }) {
  ...
  return (
    <div className="relative mx-auto w-full max-w-[340px] lg:mx-0 lg:justify-self-end" aria-label={app.eyebrow}>
      <div className="absolute -left-12 top-20 hidden h-56 w-56 rounded-full border border-brand-300/35 lg:block" aria-hidden />
      <div className="relative rounded-[42px] border border-white/15 bg-ink-muted p-2.5 shadow-dialog">
        <div className="overflow-hidden rounded-[33px] bg-canvas text-ink">
          ...
          <div className="px-5 pb-5 pt-7">
            ...
            <div className="mt-6 rounded-[18px] bg-brand-700 p-5 text-white">
              <p className="m-0 font-mono text-[9px] uppercase tracking-[0.16em] text-brand-200">{app.mockup.priority}</p>
...
function HrReportsPreview({ copy }) {
  ...
  return (
    <div className="overflow-hidden rounded-[22px] border border-ink/10 bg-surface shadow-dialog" aria-label={report.label}>
      <div className="flex items-start justify-between gap-4 border-b border-ink/8 px-5 py-4 sm:px-6">
        <div><p className="m-0 font-mono text-[9px] uppercase tracking-[0.16em] text-brand-600">{report.label}</p><h3 className="mb-0 mt-1.5 font-ui text-base font-semibold text-ink">{report.title}</h3></div>
        <span className="rounded-full border border-ink/10 px-2.5 py-1 font-mono text-[9px] text-ink-muted">{report.period}</span>
      </div>
      <div className="grid gap-px bg-ink/8 sm:grid-cols-3">{report.metrics.map((metric) => <div key={metric.label} className="bg-surface px-5 py-5"><p className="m-0 text-xs text-ink-muted">{metric.label}</p><div className="mt-2 flex items-end justify-between gap-2"><strong className="font-display text-2xl font-normal text-ink">{metric.value}</strong><span className="font-mono text-[10px] text-success">{metric.trend}</span></div></div>)}</div>
      <div className="grid gap-5 p-5 sm:grid-cols-[1.35fr_.65fr] sm:p-6">
        ...
        <div className="rounded-card border border-warning/25 bg-warning/5 p-4"><div className="mb-4 flex h-9 w-9 items-center justify-center rounded-control bg-warning/10 text-warning"><Icon name="chart" className="h-4 w-4" /></div><p className="m-0 font-mono text-[9px] uppercase tracking-[0.1em] text-ink-faint">{report.alertTitle}</p><p className="mb-0 mt-2 text-xs font-medium leading-5 text-ink">{report.alert}</p></div>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-canvas font-ui text-ink" lang={localeHtmlLang(locale)}>
      ...
      <ContentEnter animKey={locale}>
        <main id="conteudo">
          ...
          <section className="mx-auto grid max-w-7xl gap-8 px-5 py-16 sm:px-8 sm:py-20 lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:gap-14" aria-labelledby="human-context-title">
            <div className="order-2 lg:order-1">
              <p className="mb-3 font-mono text-2xs uppercase tracking-[0.2em] text-brand-600">{u.humanContextEyebrow}</p>
              <h2 id="human-context-title" className="m-0 max-w-xl font-display text-3xl font-normal leading-[1.08] tracking-[-0.025em] text-ink sm:text-4xl">{u.humanContextTitle}</h2>
              <p className="mb-0 mt-5 max-w-xl text-base leading-7 text-ink-muted sm:text-lg">{u.humanContextBody}</p>
            </div>
            <div className="order-1 overflow-hidden rounded-[24px] border border-brand-200/70 bg-brand-50 shadow-sm lg:order-2">
              <img src="/brand/landing-people-context-v1.png" alt={u.humanContextAlt} className="aspect-[3/2] h-full w-full object-cover" loading="lazy" />
            </div>
          </section>
          ...
          <section id="relatorios" className="border-t border-ink/8 bg-surface py-20 sm:py-28" aria-labelledby="reports-title"><div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[.82fr_1.18fr] lg:items-center"><div><SectionHeading label={copy.hrReports.label} title={copy.hrReports.title} body={copy.hrReports.body} id="reports-title" /><div className="mt-8 divide-y divide-ink/10 border-y border-ink/10">{copy.hrReports.groups.map((group) => <div key={group.title} className="flex gap-4 py-5"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-brand-50 text-brand-700"><Icon name={group.icon} className="h-4 w-4" /></span><div><h3 className="m-0 text-sm font-semibold text-ink">{group.title}</h3><p className="mb-0 mt-1 text-sm leading-6 text-ink-muted">{group.body}</p></div></div>)}</div><p className="mb-0 mt-6 text-sm leading-6 text-ink-muted">{copy.hrReports.delivery}</p><p className="mb-0 mt-3 flex gap-2 text-xs leading-5 text-ink-faint"><Icon name="help" className="mt-0.5 h-4 w-4 shrink-0" />{copy.hrReports.guardrail}</p></div><HrReportsPreview copy={copy} /></div></section>
          <section id="app-colaborador" className="overflow-hidden border-y border-ink/8 bg-ink py-20 text-white sm:py-28" aria-labelledby="employee-app-title"><div className="mx-auto grid max-w-7xl gap-14 px-5 sm:px-8 lg:grid-cols-[1fr_.72fr] lg:items-center"><div className="max-w-2xl"><p className="mb-3 font-mono text-2xs uppercase tracking-[0.22em] text-brand-300">{copy.employeeApp.eyebrow}</p><h2 id="employee-app-title" className="m-0 font-display text-4xl font-normal leading-[1.04] tracking-[-0.03em] text-white sm:text-5xl">{copy.employeeApp.title}</h2><p className="mb-0 mt-6 text-base leading-7 text-white/65 sm:text-lg">{copy.employeeApp.body}</p><p className="mb-0 mt-6 inline-flex items-center gap-2 rounded-full border border-brand-300/35 bg-brand-500/10 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.08em] text-brand-200"><span className="h-1.5 w-1.5 rounded-full bg-warning" />{copy.employeeApp.status}</p><ul className="mb-0 mt-8 grid list-none gap-4 p-0 sm:grid-cols-2">{copy.employeeApp.features.map((item) => <li key={item} className="flex gap-3 text-sm leading-6 text-white/80"><span className="mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-500/20 text-brand-200"><Icon name="check" className="h-3.5 w-3.5" /></span>{item}</li>)}</ul><p className="mb-0 mt-8 max-w-xl border-t border-white/10 pt-5 text-xs leading-5 text-white/45">{copy.employeeApp.note}</p></div><EmployeeAppPreview copy={copy} /></div></section>
