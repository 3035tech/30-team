...
export async function GET(request, { params }) {
  ...
  if (!row) return apiError(request, 'NOT_FOUND', 404);
...
export async function POST(request, { params }) {
  ...
  if (!isCompanyLogoStorageConfigured()) {
    return apiError(request, 'STORAGE_NOT_CONFIGURED', 503);
  }
  ...
  try {
    ...
    return apiError(request, 'INVALID_LOGO_TYPE', 400);
  }
  ...
  try {
    ...
    if (code === 'STORAGE_NOT_CONFIGURED') return apiError(request, 'STORAGE_NOT_CONFIGURED', 503);
    if (code === 'INVALID_LOGO_TYPE') return apiError(request, 'INVALID_LOGO_TYPE', 400);
    if (code === 'INVALID_LOGO_SIZE') return apiError(request, 'INVALID_LOGO_SIZE', 400);
    throw e;
  }
